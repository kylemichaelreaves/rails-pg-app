class Property < ApplicationRecord
  ...
  def ensure_owner_mailing_address
    self.owner_full_mailing_address = [owner_mailing_address, city_state_zip].compact.join(', ') if owner_mailing_address.nil?
  end

  def get_coordinates
    result = geocoder_search(property_full_address)
    if result.length > 1
      res = result.select { |p| get_municipality.in?(p.data['display_name']) and p.data['type'] == 'residential' }.first
